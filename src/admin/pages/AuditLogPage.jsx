...
import { C } from "../../theme";
import { subscribeAudit } from "../auditLog";
...
export function AuditLogPage(){
  ...
  const load = async () => {
    ...
    setLogs((data||[]).map(r=>({ id:r.id, action:r.action, detail:r.detail, time:r.created_at?new Date(r.created_at).toLocaleString("zh-TW"):"" })));
    setLoading(false);
  };
  useEffect(()=>{ load(); }, []);
  // 本次 session 有新操作 → 稍等 DB 寫入完成後重抓
  useEffect(()=> subscribeAudit(()=>{ setTimeout(load, 600); }), []);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14, maxWidth:640 }}>
      ...
      <div style={{ background:C.blueBg, border:`1.5px solid ${C.blue}30`, borderRadius:12, padding:"11px 14px", fontSize:12, color:C.textMid, lineHeight:1.7 }}>
        ✅ 日誌已持久化保存（audit_logs 表，僅業者可讀），重整/重登不會消失。顯示最近 200 筆。
      </div>
