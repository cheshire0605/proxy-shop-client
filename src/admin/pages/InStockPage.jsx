...
export function InStockPage(){
  ...
  const load = async () => {
    ...
    (data||[]).forEach(p=>(p.variants||[]).forEach(v=>{
      ...
      list.push({ id:v.id, product:p.name, image:p.image, spec:v.spec, price:v.price, stock:s, orig:s });
    }));
  ...
  const commit = async (row, val) => {
    ...
    const { error } = await supabase.from("product_variants").update({ stock:newStock, status:newStock<=0?"sold_out":"on", updated_at:new Date().toISOString() }).eq("id", row.id);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12, maxWidth:680 }}>
      ...
      {loading ? <div style={{ color:C.muted, padding:20 }}>載入中…</div> :
       ...
       rows.map(row=>{
        const out = row.stock<=0, low = row.stock>0 && row.stock<=3;
        return (
