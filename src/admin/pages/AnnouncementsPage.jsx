...
export function AnnouncementsPage(){
  ...
  const startEdit = (a) => { setEditing(a.id); setTitle(a.title||""); setContent(a.content||""); };
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14, maxWidth:640 }}>
      ...
      {editing!==null && (
        <div style={{ background:C.surface, border:`2px solid ${C.accent}`, borderRadius:16, padding:18, display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            <label style={{ fontSize:12, color:C.muted, fontWeight:700, display:"block", marginBottom:6 }}>標題</label>
            <input value={title} onChange={e=>setTitle(e.target.value)} maxLength={60} placeholder="例：第一天行程公告（選填）"
              style={{ ...inp, lineHeight:1.4, resize:"none" }}/>
          </div>
          <div>
          ...
          {content && (
            <div style={{ background:C.amberBg, borderLeft:`4px solid ${C.amber}`, borderRadius:"0 10px 10px 0", padding:"12px 14px" }}>
              ...
              {title && <div style={{ fontSize:13, fontWeight:700, color:C.textMid, marginBottom:4 }}>📢 {title}</div>}
        ...
        <div key={a.id} style={{ background:C.surface, border:`1.5px solid ${editing===a.id?C.accent:C.border}`, borderRadius:16, overflow:"hidden", boxShadow:C.shadow, opacity:editing!==null&&editing!==a.id?.5:1 }}>
          <div style={{ padding:"12px 16px", display:"flex", justifyContent:"space-between", alignItems:"center", gap:10, borderBottom:`1px solid ${C.border}` }}>
            <div style={{ minWidth:0 }}>
              {a.title && <div style={{ fontSize:13, fontWeight:700, color:C.text, marginBottom:2, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>📢 {a.title}</div>}
