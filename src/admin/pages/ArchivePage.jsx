...
export function ArchivePage(){
  ...
  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from("orders").select("*, items:order_items(*)").eq("archived", true).order("updated_at",{ascending:false});
    setOrders(data || []);
    setLoading(false);
  };
  useEffect(()=>{ load(); }, []);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12, maxWidth:720 }}>
      ...
      {loading ? <div style={{ color:C.muted, padding:20 }}>載入中…</div> :
       !filtered.length ? <div style={{ textAlign:"center", padding:"40px 0", background:C.surface, borderRadius:C.r, color:C.muted }}><div style={{ fontSize:32, marginBottom:8 }}>📭</div>沒有封存訂單</div> :
       filtered.map(o=>(
